...
//_________________________________________________________________________________________________
MIDRatesComputerDevice::~MIDRatesComputerDevice()
{
...
bool MIDRatesComputerDevice::HandleData(FairMQMessagePtr& msg, int /*index*/)
{
  ...
  if (MIDRatesComputerDevice::ShouldComputeRates(digitType::kPhysics))
    MIDRatesComputerDevice::ComputeAllRates();

  bool returnValue = true;
  // Try to send newly computed rates and catch errors
  switch (MIDRatesComputerDevice::SendRates<uint64_t>()) {
    case kShortMsg:
  ...
  MIDRatesComputerDevice::ResetCounters(0, digitType::kPhysics);

  return returnValue;
}

//_________________________________________________________________________________________________
void MIDRatesComputerDevice::ResetCounters(uint64_t newStartTS, digitType type)
{
...
//_________________________________________________________________________________________________
bool MIDRatesComputerDevice::ShouldComputeRates(digitType type)
{
...
//_________________________________________________________________________________________________
void MIDRatesComputerDevice::ComputeRate(stripMapping* strip)
{
...
//_________________________________________________________________________________________________
void MIDRatesComputerDevice::ComputeAllRates()
{
  auto tStart = std::chrono::high_resolution_clock::now();
