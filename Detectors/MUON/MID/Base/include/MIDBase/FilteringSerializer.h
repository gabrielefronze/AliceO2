...
class FilteringSerializer
{
 public:
  ...
  {
    return sizeof(uint32_t) * (fHeader.size()) +
           this->GetPayloadSize(); // The +1 is the value representing the number of digits
  }

  inline size_t GetPayloadSize()
  {
    return sizeof(uint32_t) * (1 + fOutputData.size()); // The +1 is the value representing the number of digits
  }
