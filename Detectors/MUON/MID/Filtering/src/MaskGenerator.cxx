...
void MaskGenerator::processData(o2::mid::CounterContainer data, o2::mid::dataType dataType)
{
  ...
  switch (dataType) {
    case o2::mid::dataType::DEAD:
      detectDead(data.deId, std::move(data[dataType]));
    case o2::mid::dataType::NOISY:
...
void MaskGenerator::detectDead(uint8_t deId, o2::mid::CounterStruct data)
{
  ...
  for (auto const& itColumn : data.columns) { // 0->7
    ...
    for (auto const &itLocalBoard : itColumn.counters) { // 0->5
      ...
      for (auto const &itScalers : itLocalBoard.counter) { // 0->16
        ...
        maskData.patterns.patterns[iLB] |= (itScalers == 0) << iBit;
    ...
    for (auto const &itPatterns : maskData.patterns.patterns) {
...
void MaskGenerator::detectNoisy(uint8_t deId, o2::mid::CounterStruct data)
{
  uint8_t columnId = 0;
  // Loop over DE columns
  for (auto const& itColumn : data.columns) { // 0->7
    ...
    for (auto const& itLocalBoard : itColumn.counters) { // 0->5
      ...
      size_t iBit = 0;
      // Loop over counters
      maskData.patterns.patterns[iLB] = 0;
      for (auto const& itScalers : itLocalBoard.counter) { // 0->16
      ...
      maskData.patterns.patterns[iLB] = ~(maskData.patterns.patterns[iLB]);
