...
BOOST_AUTO_TEST_CASE(LoadNoisy)
{
  ...
  for (auto const& pat : patternsCol0) {
    std::cout << std::bitset<16>(mask.patterns.patterns[iPat]) << " mask represents NOT noisy channels in "
              << std::bitset<16>(pat) << std::endl;
    BOOST_TEST(mask.patterns.patterns[iPat] == (uint16_t)(~pat));
    iPat++;
  }
}
...
BOOST_AUTO_TEST_CASE(LoadDead)
{
  ...
  uint iPat = 0;
  for (auto const& pat : patternsCol0) {
    std::cout << std::bitset<16>(mask.patterns.patterns[iPat]) << " mask represents dead channels in "
              << std::bitset<16>(pat) << std::endl;
    BOOST_TEST(mask.patterns.patterns[iPat] == (uint16_t)(~pat));
    iPat++;
  }
