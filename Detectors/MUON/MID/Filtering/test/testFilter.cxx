...
BOOST_AUTO_TEST_CASE(LoadMask)
{
  ...
  std::vector<uint16_t> patternsCol0 = { 1, 29820, 56701, 44471, 1024 * 64 - 1 };
...
BOOST_AUTO_TEST_CASE(ApplyMask)
{
  ...
  std::vector<uint16_t> rawDataPatterns = { 0xFFFF, 0xBACD, 0xFFFF, 0x1234, 0xFFFF };
  rawData.columnId = 4;
  rawData.deId = 62;
  ...
  size_t iMask = 0;
  for (auto const& mskPatternIt : maskedData.patterns.patterns) {
...
BOOST_AUTO_TEST_CASE(ApplyEmptyMask)
{
  ...
  auto rawData = o2::mid::ColumnData();
  std::vector<uint16_t> rawDataPatterns = { 0xFFFF, 0xABCD, 0xFFFF, 0x1234, 0xFFFF };
  rawData.columnId = 4;
  rawData.deId = 62;
  rawData.setPatterns(rawDataPatterns);
  ...
  for (auto const& mskPatternIt : maskedData.patterns.patterns) {
    ...
    BOOST_TEST(mskPatternIt == rawData.patterns.patterns[iPat]);
    iPat++;
  }
}
